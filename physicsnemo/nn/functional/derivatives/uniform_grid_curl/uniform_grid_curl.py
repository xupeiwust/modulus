...
from ._torch_impl import uniform_grid_curl_torch
from ._warp_impl import uniform_grid_curl_warp


class UniformGridCurl(FunctionSpec):
    r"""Compute periodic curl on a uniform grid.

    This functional accepts channel-first vector fields with shape
    ``(dim, *grid_shape)`` for 2D or 3D uniform grids. For 2D inputs, it
    returns scalar vorticity. For 3D inputs, it returns the channel-first
    vector curl.
    ...
    _BENCHMARK_CASES = (
        ...
        ("3d-80x80x80-o4", (80, 80, 80), 0.02, 4),
    )
    ...
    def warp_forward(
        ...
        """Dispatch uniform-grid curl to the Warp backend."""
        return uniform_grid_curl_warp(
    ...
    def torch_forward(
        ...
        """Dispatch uniform-grid curl to eager PyTorch."""
        return uniform_grid_curl_torch(
            vector_field=vector_field,
            spacing=spacing,
            order=order,
        )
...
uniform_grid_curl = UniformGridCurl.make_function("uniform_grid_curl")
